...
const logger = createLogger('db:clients:ibmdb');
...
export default async function (server, database) {
  ...
  const conn = {
    pool: new ibmdb.Pool(),
    connStr: dbConfig.dsn
  };

  // light solution to test connection with with the server
  await driverExecuteQuery(conn, { query: 'SELECT 1 FROM SYSIBM.SYSDUMMY1;' });
...
async function getSchema(conn) {
  const sql = 'SELECT CURRENT_SCHEMA AS schema FROM SYSIBM.SYSDUMMY1';
...
export async function listTableColumns(conn, database, table) {
  const sql = `
    SELECT NAME as column_name, COLTYPE as data_type
    FROM SYSIBM.SYSCOLUMNS
    WHERE TBCREATOR = database()
    AND TBNAME = ?
  `;
...
function configDatabase(server, database) {
  ...
  const dsn = 'DRIVER={DB2};DATABASE=' + config.database +
        ';UID=' + config.user +
        ';PWD=' + config.password +
...
function driverExecuteQuery(conn, queryArgs) {
  const runQuery = (connection) => new Promise((resolve, reject) => {
    connection.query(queryArgs.query, queryArgs.params, (err, data, fields) => {
      if (err && err.code === ibmdbErrors.EMPTY_QUERY) return resolve({});
...
async function runWithConnection({ connStr, pool }, run) {
  let rejected = false;
  return new Promise((resolve, reject) => {
    ...
    pool.open(connStr, async (errPool, connection) => {
      if (errPool) {
        rejectErr(errPool);
        return;
      }
